"""Deterministic Stage 2 seven-day scenario gate."""
...
from tools.scenario_harness import load_scenario, run_stage2_seven_day
...
SCENARIO = ROOT / "backend" / "tests" / "fixtures" / "stage2_seven_day_world.toml"
...
@pytest.mark.asyncio
async def test_stage2_seven_day_world(
    uow_factory: async_sessionmaker[AsyncSession],
) -> None:
    spec = load_scenario(SCENARIO)
    result = await run_stage2_seven_day(
    ...
    assert len([item for item in result.task_trace if item.startswith("day:")]) == 7
    assert len(result.state_hashes) == 70

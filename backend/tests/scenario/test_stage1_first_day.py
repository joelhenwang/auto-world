"""Deterministic Stage 1 first-day scenario gate."""
...
from tools.scenario_harness import load_scenario, run_stage1_first_day
...
SCENARIO = ROOT / "backend" / "tests" / "fixtures" / "stage1_first_day.toml"
...
@pytest.mark.asyncio
async def test_stage1_first_day_scenario(
    uow_factory: async_sessionmaker[AsyncSession],
) -> None:
    spec = load_scenario(SCENARIO)
    result = await run_stage1_first_day(
    ...
    assert [item.split(":")[-1] for item in result.task_trace[1:]] == [
        "dawn",
        "morning",
        "evening",
    ]
    assert len(result.state_hashes) == 3
    assert len(set(result.state_hashes)) == 3
    assert len(result.model_calls) == 10
